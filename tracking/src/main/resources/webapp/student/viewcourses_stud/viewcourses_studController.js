...
angular.module('trackingWebApp')
	.controller('viewcourses_studController', viewcourses_studController);

function viewcourses_studController($scope, $rootScope, $state, $filter, dialogs, restAPIService,$timeout){
	$scope.courses = [];
	$scope.batches = [];
	$scope.favCourses = [];
	...
	getPoularCourses();
	 ...
	 $timeout(function () {
		 if($scope.coursesToShow.length < 10) {
			 $('#stCoursesToShowDataTable_paginate').attr("style","display:none;");
		 }
    }, 1000);
